...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Authors: Anssi Kanervisto {anssk [at] uef [dot] fi}
...
% Date: 7.10.19
%
% This version of original code v1.0 is designed to work on Octave 5.0
%
...
%%% UNIFORM RESAMPLING
kl = (B*log2(1+1/d));
% Original Matlab version. Octave does not support 
% the request resampling method
%[Ures_LogP_absCQT, Ures_FreqVec] = resample(LogP_absCQT,...
%    FreqVec,1/(fmin*(2^(kl/B)-1)),1,1, "spline");

% Attempt to replicate previous in Octave using spline
% This is accurate to above one withing five digits
target_sample_rate = 1/(fmin*(2^(kl/B)-1));
% Uniform samples in the tx
Ures_FreqVec = linspace(
  min(FreqVec),
  max(FreqVec),
  ceil(max(FreqVec) - min(FreqVec)) * target_sample_rate
);
Ures_LogP_absCQT = spline(FreqVec, LogP_absCQT, Ures_FreqVec)';
